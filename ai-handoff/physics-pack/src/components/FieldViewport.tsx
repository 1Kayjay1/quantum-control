...
function FieldObjectMesh({
  ...
  const labelText = object.checkpointOrder
    ? `CP${object.checkpointOrder} ${object.name}`
    : object.name
  ...
  if (object.type === 'ring') {
    return (
      ...
      >
        <mesh>
          <torusGeometry args={[size.x * WORLD_SCALE * 0.33, 0.018, 14, 28]} />
  ...
  if (object.type === 'gate') {
    ...
    return (
      ...
      >
        ...
        <mesh position={[0, 0, 0]}>
          <boxGeometry args={[width * 0.82, height * 0.82, depth * 1.15]} />
...
function SceneView({
  ...
  return (
    <>
      ...
      <OrbitControls
        makeDefault
        enabled={!dragState && !transformDragging}
        enableDamping
        dampingFactor={0.08}
        enablePan
        screenSpacePanning
    ...
    <section className="viewport-shell">
      ...
      <div className="viewport-toolbar">
        <div className="viewport-toolbar__group">
          <span className="viewport-toolbar__label">Replay</span>
          <button
            type="button"
          ...
          <span className="viewport-toolbar__meta">
            Frame {run ? `${currentFrame}/${totalFrames}` : '0/0'}
          </span>
        </div>

        <div className="viewport-toolbar__group" role="group" aria-label="Object transform mode">
          <span className="viewport-toolbar__label">Transform</span>
          <button
            type="button"
            className={`button button--ghost button--small${transformMode === 'translate' ? ' button--active' : ''}`}
          ...
          <button
            type="button"
            className={`button button--ghost button--small${transformMode === 'rotate' ? ' button--active' : ''}`}
          ...
          <button
            type="button"
            className={`button button--ghost button--small${transformMode === 'scale' ? ' button--active' : ''}`}
